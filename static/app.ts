...
const GMAPS_EMBED_API_KEY = (window as { GMAPS_EMBED_API_KEY?: string })
  .GMAPS_EMBED_API_KEY;

const imageForm = document.querySelector<HTMLFormElement>("#image-form");
const videoForm = document.querySelector<HTMLFormElement>("#video-form");
const agentForm = document.querySelector<HTMLFormElement>("#agent-form");
const mergeForm = document.querySelector<HTMLFormElement>("#merge-form");
const imageResult = document.querySelector<HTMLPreElement>("#image-result");
const videoResult = document.querySelector<HTMLPreElement>("#video-result");
const agentLog = document.querySelector<HTMLDivElement>("#agent-log");
const agentFinal = document.querySelector<HTMLPreElement>("#agent-final");
const agentPreview = document.querySelector<HTMLDivElement>("#agent-preview");
const mergeResult = document.querySelector<HTMLPreElement>("#merge-result");
const mergeVideo = document.querySelector<HTMLVideoElement>("#merge-video");
const adBadge = document.querySelector<HTMLDivElement>("#ad-badge");
const adProgress = document.querySelector<HTMLDivElement>("#ad-progress");
let lastGmapsResult: JsonRecord | null = null;

function setResult(target: HTMLPreElement | null, payload: JsonRecord) {
  if (!target) return;
...
async function streamAgent(prompt: string) {
  ...
  if (agentFinal) agentFinal.textContent = "";
  if (agentPreview) agentPreview.innerHTML = "";
  ...
  while (true) {
    ...
    while (index !== -1) {
      ...
      if (raw.startsWith("data:")) {
        ...
        const payload = JSON.parse(jsonText) as JsonRecord;
...
function renderAgentEvent(payload: JsonRecord) {
  ...
  if (previewTargets.length && agentPreview) {
    previewTargets.forEach((target) => {
      ...
      if (node) agentPreview.appendChild(node);
    });
  }
  ...
  if (type === "final") {
    ...
    if (lastGmapsResult && agentPreview) {
      ...
      if (groundingNode) agentPreview.appendChild(groundingNode);
    }
    return;
  }
...
function storeGmapsResult(payload: JsonRecord) {
  if (payload.type === "tool_result" && payload.tool === "tool_gmaps_grounding") {
    const output = payload.output as JsonRecord | undefined;
    if (output) lastGmapsResult = output;
    return;
  }
  if (payload.type === "final") {
    const result = payload.result as JsonRecord | undefined;
    const response = (result?.function_response || null) as JsonRecord | null;
    if (response?.name === "tool_gmaps_grounding") {
      const output = response.response as JsonRecord | undefined;
      if (output) lastGmapsResult = output;
    }
  }
}

function renderGmapsGrounding(output: JsonRecord): HTMLElement | null {
  ...
  const details = (output.details || {}) as JsonRecord;
...
function formatLatLng(value: unknown): string | null {
...
function formatArray(value: unknown): string | null {
...
function formatRating(rating: unknown, total: unknown): string | null {
...
function formatMapLink(placeId: unknown): string | null {
...
function renderGroundingMap(
  placeId: unknown,
  location: unknown
): HTMLElement | null {
  if (!GMAPS_EMBED_API_KEY) return null;
...
function buildEmbedUrl(placeId: unknown, location: unknown): string | null {
...
function collectPreviewTargets(payload: JsonRecord): PreviewTarget[] {
  ...
  const walk = (value: unknown, depth: number) => {
    ...
    if (typeof value === "string") {
      ...
      if (!maybe || seen.has(maybe)) return;
      seen.add(maybe);
...
function collectText(payload: JsonRecord): string {
  ...
  const walk = (value: unknown, depth: number) => {
    if (depth > 4) return;
    if (typeof value === "string") {
      if (value.length < 5000) parts.push(value);
      return;
    }
    if (Array.isArray(value)) {
      value.forEach((item) => walk(item, depth + 1));
...
function extractPathsFromText(text: string): string[] {
  const matches = text.match(
    /[\\w./-]+\\.(?:png|jpg|jpeg|webp|mp4|wav|mp3|aac)/gi
  );
  return matches ? Array.from(new Set(matches)) : [];
}

function addTargetFromValue(
...
function fileToUrl(path: string) {
  return `/files?path=${encodeURIComponent(path)}`;
}

function renderPreview(target: PreviewTarget): HTMLElement | null {
...
function wireAdIndicator(video: HTMLVideoElement, start: number, end: number) {
  const update = () => {
...
function updateAdTimeline(
  start: number,
  end: number,
  video: HTMLVideoElement
) {
  const apply = () => {
    if (!adProgress) return;
